...
const checks = [
  ...
  ["navigation is server-static with vanilla interaction runtime", layout.includes("<SiteNav />") && layout.includes('src="/site-nav-runtime.js?v=123.42"') && !nav.includes('"use client"') && nav.includes('className="minimal-site-nav one-page-nav"') && css.includes("body:has(.legal-page) .minimal-site-nav{display:none!important}")],
  ...
  ["GitHub Pages preview launcher builds before serving", read("START-PASS-123.42-MOBILE-MENU-NATIVE-TAP.bat").includes("npm run build:pages") && read("START-PASS-123.42-MOBILE-MENU-NATIVE-TAP.bat").includes("npm run preview:pages")],
