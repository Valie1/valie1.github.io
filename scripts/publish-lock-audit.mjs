...
const checks = [
  ...
  ["navigation is server-static with vanilla interaction runtime", layout.includes("<SiteNav />") && layout.includes('src="/site-nav-runtime.js?v=123.19"') && !nav.includes('"use client"') && nav.includes('className="minimal-site-nav one-page-nav"') && css.includes("body:has(.legal-page) .minimal-site-nav{display:none!important}")],
  ...
  ["GitHub Pages preview launcher builds before serving", read("START-PASS-123.19-MOBILE-HERO-LIVE-PARITY.bat").includes("npm run build:pages") && read("START-PASS-123.19-MOBILE-HERO-LIVE-PARITY.bat").includes("npm run preview:pages")],
